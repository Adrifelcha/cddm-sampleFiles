...
myCDDMsimData.defineVector <- function(mu.1,mu.2){
  DriftAngle <- 1/tan((mu.2)/(mu.1))
  DriftLength <- sqrt((mu.1^2)+(mu.2^2))
  
  vectorParameters <- as.data.frame(cbind(DriftAngle,DriftLength))
  colnames(vectorParameters) <- c("theta0","drift")
...
myCDDMsimData.simData <- function(trials, mu.1, mu.2, std.driftrate, thresh){
      ...
      y <- myCDDMsimData.defineVector(mu.1,mu.2)
      parameters <- as.data.frame(cbind(y,thresh))
      colnames(parameters) <- c(colnames(y),"bound")
      
      output <- list(data,parameters)
      names(output) <- c("simData","trueParameters")
